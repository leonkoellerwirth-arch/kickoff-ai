...
# --- Shell surface ---
# This repo is neither python nor web: it is a shell toolchain. Without this
# block the gate would find nothing to check and pass vacuously — a green gate
# that verifies nothing is worse than no gate.
# These are the same checks .github/workflows/validate.yml runs, so local and
# CI cannot drift apart.
if [ -f bootstrap.sh ] && [ -d automation/bin ]; then
  # Collect every bash file once: *.sh plus the extension-less commands.
  find . -name "*.sh" -not -path "./.git/*" -not -path "./local/*" > /tmp/gate_sh.txt 2>/dev/null
  find automation/bin -maxdepth 1 -type f 2>/dev/null | while IFS= read -r f; do
    head -1 "$f" 2>/dev/null | grep -q '#!/.*bash' && echo "$f"
  done >> /tmp/gate_sh.txt

  # The loop runs in a subshell: check() evaluates its argument with eval in the
  # current shell, so a bare `exit 1` would terminate gate.sh itself and silently
  # skip every remaining check.
  check "bash -n clean (all shell files)" \
    "( while IFS= read -r f; do [ -z \"\$f\" ] && continue; bash -n \"\$f\" || exit 1; done < /tmp/gate_sh.txt )"

  if command -v shellcheck >/dev/null 2>&1; then
    check "shellcheck -S warning clean" "xargs shellcheck -S warning < /tmp/gate_sh.txt"
  else
    skip "shellcheck skipped (not installed — brew install shellcheck)"
  fi

  [ -d automation/launchd ] && check "plutil — LaunchAgent plists valid" \
    "find automation/launchd -name '*.plist' -exec plutil -lint {} +"

  check "workflow + registry YAML parses" \
    "for y in .github/workflows/*.yml manifests/tools.yaml; do python3 -c \"import yaml,sys;yaml.safe_load(open('\$y'))\" || exit 1; done"

  # Every run: block in the workflows must be valid shell. An indented heredoc
  # delimiter inside a YAML block silently breaks the whole step.
  check "workflow run-blocks are valid shell" \
    "python3 -c \"
import yaml,subprocess,tempfile,os,glob,sys
bad=0
for wf in glob.glob('.github/workflows/*.yml'):
    ...
    for job in (d.get('jobs') or {}).values():
        for s in (job.get('steps') or []):
            sc=s.get('run')
            if not sc: continue
            t=tempfile.NamedTemporaryFile('w',suffix='.sh',delete=False); t.write(sc); t.close()
            if subprocess.run(['bash','-n',t.name],capture_output=True).returncode: bad+=1
            os.unlink(t.name)
sys.exit(1 if bad else 0)\""

  [ -x automation/bin/up2date ] && check "registry consistent with install lists" \
