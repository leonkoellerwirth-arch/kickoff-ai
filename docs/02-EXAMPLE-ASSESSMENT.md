# Example Assessment — Gap Analysis of a Representative macOS Developer Workstation

> **Historical Snapshot — Example Report**
>
> This document records the findings from a gap analysis conducted against a real, organically grown macOS developer workstation in August 2026. The identified deficiencies were addressed after the survey. This is a historical snapshot and does not reflect the current state of any particular system. It is published as a worked example of the methodology described in this repository — the patterns and recommended remediations transfer to any similarly configured machine.

This document has two parts:

- **Part A:** Concrete, verified defects found during the scan — what was broken or working suboptimally.
- **Part B:** What professional AI developers with a similar profile do that was absent from the scanned system.

At the end: a prioritized Top 10 list — the highest leverage per hour invested.

The findings are written so they transfer to any similarly evolved machine — as patterns, not as characterizations of a specific individual.
...
**Finding:** An agent gateway tool (OpenClaw) was registered three times: as an npm global package, as a pnpm global link, and as a LaunchAgent. An environment variable `OPENCLAW_PATH` pointed to a directory (`~/dev/openclaw`) that **no longer existed**. Four shell aliases and two completion blocks in `.zshrc` pointed at nothing. The pnpm global link pointed to the same nonexistent path.
...
**Impact:** Every shell startup potentially generated silent errors. `pnpm` executed a link call to a dead path. The LaunchAgent attempted to start the gateway at every login — and failed.
...
**Finding:** `M2_HOME=$HOME/dev/maven` was set in `.zshrc`. The directory did not exist — Maven ran from the Homebrew prefix (`/opt/homebrew/bin/mvn`). `M2_HOME` was also no longer necessary for current Maven; the value was historically required for Maven 2 projects.
...
**Finding:** `source <(ng completion script)` in `.zshrc` loaded the Angular CLI completion. Angular CLI was not installed. The command failed silently (suppressed) but slowed down shell startup and was misleading for anyone reading the `.zshrc`.
...
**Finding:** Multiple shell files produced duplicate entries:
- `/usr/local/bin` appeared multiple times in PATH
- `/usr/bin` appeared multiple times in PATH
- `.local/bin` appeared multiple times in PATH (once in `.profile`, once in `.zshrc`)
- VS Code PATH block was duplicated in `.zprofile`
- OpenClaw completion block was duplicated in `.zshrc`
...
**Impact:** PATH grew longer than necessary, shell startup slowed down, debugging became harder.
...
**Finding:** `/usr/bin/git` (Apple CLT version) preceded `/opt/homebrew/bin/git` (the most recent git version) in PATH order. As a result, `which git` returned `/usr/bin/git`, and features of newer git versions were unavailable. `git-lfs` and certain hooks may also have been affected.
...
**Finding:** `python3` in the shell pointed to conda-base 3.10.9. The conda-base environment contained packages from various unrelated projects (machine learning, scientific computing, web scraping, flashcard software). conda was at version 22.11.1 — two major versions behind at the time of the scan.
...
**Impact:** Project isolation did not work in practice when `python3` globally pointed to conda-base. Newly set-up projects uncontrollably inherited packages from the base environment.

**Fix:** `config/zshrc` does not auto-activate conda-base. `python3` then points to Homebrew Python or is managed per-project by `uv python`. `conda activate <env>` continues to work explicitly. Do **not** delete conda-base — existing project-specific environments stay intact.
**Priority:** High.
...
**Finding:** `dev/base` contained a `.pre-commit-config.yaml` (with a gitleaks hook and additional checks) and a `.gitleaks.toml`. The `pre-commit` tool itself was **not installed** on the scanned system — not via pip, pipx, or brew. The hooks therefore never ran.
...
**Impact:** No commit underwent secret scanning or code quality checks. The central safety net in `dev/base` was inactive.
...
**Finding:** `~/.gitignore_global` existed but was empty (0 bytes). It was also not registered as `core.excludesfile` in the global git configuration — meaning git ignored it entirely.
...
**Impact:** `.DS_Store`, `.env`, `node_modules`, `__pycache__`, `.idea`, `.vscode/` and other system-specific files were not ignored globally. They had to be listed in every individual repo's `.gitignore` — or they could end up in the repo accidentally.
...
**Finding:** The scanned system carried a single SSH key in RSA format. No GPG keys were present (gnupg was installed, but the trust database was empty at scan time). Commits were not signed (`commit.gpgsign` was not set).
...
**Impact:** RSA is not insecure, but ed25519 is the current standard. Without commit signing there was no cryptographic proof that commits originated from the registered identity — this matters especially for public repos and GitHub contributions.
...
**Finding:** `git config --global maintenance.repo` pointed to a path that no longer existed (a company repo path from an earlier configuration). git maintenance ran daily/hourly/weekly via LaunchAgents (`org.git-scm.git.daily/hourly/weekly`) — and failed silently.
...
**Finding:** `~/Library/LaunchAgents/homebrew.mxcl.mariadb.plist` existed even though MariaDB was no longer installed. The agent attempted to start MariaDB at every login — and failed.
...
**Finding:** Ollama was present through three separate channels: as a brew formula (`ollama`), as a brew cask (`ollama-app`), and as a running Docker container (in the context of an AI project). All three could run in parallel. The brew formula and cask are different things — the cask is a native macOS app with a menu bar icon, the formula is a CLI service.
...
**Impact:** Potentially two Ollama instances competed for port 11434 — race condition on `ollama serve`. The Docker container had no Metal GPU access on macOS and ran CPU-only.
...
**Finding:** At the time of measurement, 16 Docker containers were running, some for weeks, including five MySQL instances for different projects. Only one container was in `exited` state. The pattern: docker-compose stacks for various projects had been started and never stopped.
...
**Impact:** Multiple GB of RAM were permanently occupied, unnecessary energy consumption occurred, and ports were blocked that other services might need.
...
### A.14 — Storage Utilization Above Critical Threshold

**Finding:** Storage utilization on the scanned system exceeded the threshold at which macOS begins to show performance degradation — less than 10% of total capacity remained free. The largest contributors were Docker images and volumes, the conda-base installation with hundreds of packages, and locally stored AI model files.
...
**Impact:** macOS shows performance degradation below 10% free storage. Pulling new models would fail. Docker builds could fail.
...
**Finding:** Claude Code was configured with `skipDangerousModePermissionPrompt`, `skipAutoPermissionPrompt`, and `defaultMode: auto`. Codex CLI had `approval_mode = "full-auto"` set and trusted 17 projects at the "trusted" level — including the entire home directory (`~`).

This meant: Claude Code and Codex could write files, delete files, and execute commands in these directories without a confirmation dialog.
...
**Impact:** With a bad prompt, a misunderstanding, or a prompt injection from processed files, an agent could perform destructive actions without asking. The risk is real — not theoretical.
...
**Finding:** A directory was named `~/dev/scripts` but contained not a general script collection but rather files from a specific project (prose files and project-specific helper scripts). Shell aliases pointed to a file (`media-tools.sh`) in this directory — that worked, but the directory name was misleading for anyone looking for setup scripts.
...
**Impact:** Naming confusion: a reader searching `~/dev/scripts` would expect scripts but find project-specific files.
...
### A.17 — Multiple GitHub Accounts Without a Defined Separation Rule

**Finding:** Multiple GitHub accounts were authenticated via `gh`, without a documented rule governing which account applied to which repositories. The SSH configuration contained only a single host block for `github.com`, without distinct aliases per account.

Verification:
```bash
gh auth status          # shows authenticated accounts
cat ~/.ssh/config       # shows whether github.com has one or two entries
```

**Impact:** Without clear separation, `git push` could accidentally use the wrong account. CI secrets and deployment keys could become mixed.
...
## Part B — What Professional AI Developers with a Similar Profile Do That Was Absent

The scanned system had the following profile: 36 frontend repositories (Vite/React/TypeScript/Tailwind + vitest), 10 Python services (FastAPI/Pydantic/pytest/ruff), LangChain/LangGraph with Anthropic/OpenAI/Ollama for AI, iOS apps, 2 additional AI agent CLIs alongside Claude Code, a custom paved road (`dev/base`), 58 repositories in total.

---

### B.1 — Day-to-Day Secrets Management: .env Files in Plaintext

**Finding:** `.env` files were stored as plaintext files on disk. No secrets backend was in place to keep the values off disk.
...
**Finding:** Backups of the shell configuration existed (`~/.zshrc_bkp`, `~/.zshrc.save`), but no dotfiles repo was present. The configuration was therefore only locally versioned and not easily deployable on a new machine.
...
**Finding:** Node was managed via nvm, Python via uv + conda, Java via brew-openjdk. No single tool tracked the complete version state. `.nvmrc` was present in only 2 of 36 frontend repositories.
...
**Finding:** `langfuse` was present as a repository (fork/clone). There was no indication it had been instrumented for any active projects.
...
**Finding:** The scan found no indication of systematic prompt testing or agent evaluation. LLM output quality was assessed manually.
...
**Finding:** Claude Code had a cost brake for subagents (`CLAUDE_CODE_SUBAGENT_MODEL=claude-sonnet-4-6`). For the other CLIs (Codex, Gemini, and any further agent tools), there was no measured or visible cost control.
...
**Finding:** `dev/base` defined a local quality gate (`./scripts/gate.sh` or `npm run verify:ci`). No CI pipeline (GitHub Actions or similar) ran the same gate on every push.
...
**Context:** 15 of 58 repositories had `docker-compose.yml`, 4 had `Makefile` — many already had build logic. The GitHub Actions integration was absent.
...
**Finding:** No Time Machine status was visible. No indication of cloud backup (Backblaze, iCloud Drive for non-dev data, etc.) was found.
...
**Leverage:** Very high — with storage at a critical level, the risk of data loss was real.
...
**Finding:** Below-threshold SSD availability was a systemic condition, not a one-time problem. Without regular hygiene it will recur.
...
**Finding:** The scan identified 58 entries in the development directory, of which an estimated 10–15 were actively used. Many repos (open-source clones: langfuse, Scrapling, Anki-Android, screenshot-to-code, and others) were cloned, possibly used once, and remained on disk.
...
## Top 10: Ten Highest-Leverage Actions
...
| 2 | **Clear storage below threshold** (A.14) | A | 1–2 h | Stability, new models possible |
...
| 10 | **Configure multiple GitHub accounts with clean SSH separation** (A.17) | A | 1 h | No wrong account on wrong repo |
